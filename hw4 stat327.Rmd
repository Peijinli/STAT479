...
```{r}
# ...
tables = readHTMLTable(getURL("https://simple.wikipedia.org/wiki/List_of_U.S._states_by_area"), header = T, trim=T)
tables=tables[[2]]
colnames(tables) <- c("Rank", "State","sq.km", "sq.miles")
tables <- tables[-c(51),]
tables$sq.miles =sub(pattern=",", replacement = "", tables$sq.miles)
tables$sq.miles <- as.numeric(tables$sq.miles)
tables
```
...
colnames(farm) <- c("State","sq.miles")
farm
```
...
```{r}
area=data.frame(state=c(1:50), farm=c(1:50), land=c(1:50))
area[,1]=farm[,1]
area[,2]=farm[,2]
...
```{r}
plot(area$farm~area$land, xlab="land area", ylab="farm area")
...
plot(area$farm~area$land, xlab="land area", ylab="farm area")
outlier=identify(area$farm~area$land)
outlier
...
```{r}
lm=lm(area$farm~area$land, area)
plot(area$farm~area$land, xlab="land area", ylab="farm area")
abline(lm)

lm2=lm(area$farm[-c(2)]~area$land[-c(2)])
abline(lm2, col="red")
legend("topleft", legend=c("LM for all ", "LM except Alaska"), col=c("black", "red"), lty=c(1,1) )
...
y=c(1:50)
for (i in 1:50){
  fit=lm(area$farm[-i]~area$land[-i])
  y[i]=fit$coefficients[1] + fit$coefficients[2] * area$land[i]
  r.jack[i]=y[i]-area$farm[i]
}
...
```{r}
ggplot(warpbreaks, aes(breaks, colour=wool))+geom_density()
...
```{r}
ggplot(warpbreaks, aes(breaks, colour=tension))+geom_density()
...
attach(faithful)
faithful
lm=lm(waiting~eruptions)
ggplot(faithful, aes(eruptions,waiting))+geom_point()+geom_line(aes(y=fitted(lm)))
```
...
```{r}
# ...
chart=readLines("http://www.imdb.com/chart/top")
chart.link=grep(pattern="^<a href=./title/tt.*",x=chart, value=T)
movie.link = sub(pattern="^<a href=.(.*).[?]pf_rd_m.*", replacement="\\1", x=chart.link)
title=c(1:250)
...
for(i in 1:250){
  ...
  a=readLines(link)
title.line=grep(pattern="- Full Cast",x=a, value=T)
title[i]=sub(pattern="<title>(.*).[0-9]{4}.*- Full Cast.*",replacement="\\1",x=title.line)
producers[[i]]=readHTMLTable(link)[[4]]
}
...
#######
producers1<-producers # to save producers. for-loop takes long time.
producers1[[1]]
for(i in 1:250){
  producers1[[i]]=producers1[[i]]$V1
}


producers1<-unlist(producers1)
most=table(producers1)

display=most[order(most, decreasing=T)]
head(display, n=5)
...
actor=list()
for(i in 1:250){
  actor.link = paste0("http://www.imdb.com", movie.link[i])
  source.code=readLines(actor.link)
  director.line=grep("Directed by", source.code)
  director[[i]]=sub(".*Directed by (.*). With.*", replacement = "\\1", x=director.line)
  actor.table=readHTMLTable("http://www.imdb.com", movie.link[i])
  actor=actor.table[[i]]$V2


  
  rating.line=grep(".*strong title", source.code)
...
do=readLines("http://www.imdb.com/title/tt0092067/")
director.line=grep(pattern=".*Directed by.*", x=do, value=T)
director[[1]]=sub(".*Directed by (.*). With.*", replacement = "\\1", x=director.line)
director[[1]]<-paste(unique(director[[1]]), collapse = ' ')
director[[1]]


actor.table=readHTMLTable("http://www.imdb.com/title/tt0092067/")
actor.table=actor.table[[1]]$V2
actor.table


year.line=grep(pattern=".*a href=.*/year/.*", x=do, value=T)
year=sub(patter=".*([0-9]{4}).*", year.line, replacement = "\\1")
year


rating.line=grep(".*strong title", do, value=TRUE)
rating=sub(".*([0-9]{1}.[0-9]{1}).*", rating.line, replacement = "\\1")
rating
