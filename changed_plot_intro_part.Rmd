...
for_plot=stock[daterange1] %>% as.tibble() %>% 
  ...
  gather('AOBC','RGR','VSTO','OLN',key="Co",value="price") %>%
  `colnames<-`(c("date", "case","company", "price"))
...
for_plot%>% ggplot(aes(y=price,x=date, color=company))+geom_point() +facet_wrap(~ case, ncol = 4, scales = "free", labeller=label_both)+geom_line()
